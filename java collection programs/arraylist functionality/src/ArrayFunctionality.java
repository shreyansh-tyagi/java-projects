...
class Displays extends MainArray {
	...
	public void displaytreeoperation(TreeSet<String> treeset) {
		...
		System.out.println("updated treeset is:" + treeset); // using pollfirst and polllast the largest and smallest
																// value is retreived and removed
		System.out.println("headset treeset is:" + treeset.headSet("ramesh")); // it take element upto ramesh excluding
																				// the ramesh
		System.out.println("headset with boolean value: " + treeset.headSet("ramesh", true)); // here true means include
																								// the element and false
																								// mean exclude the
																								// element
		System.out.println("subset in treeset: " + treeset.subSet("set element added", false, "rajat", true));
		System.out.println("tailset in treeset: " + treeset.tailSet("set element added", false));// it will give element
																									// upto 'set element
																									// added' after
																									// excluding it as
																									// false vlaue is
...
class LinkedLIST extends MainArray {
	public List<String> reverse(List<String> re) {
		 ...
		 * while(i.hasNext()) { System.out.print(" "+i); // one way reversing the list
		 * using iterator descending function where we have to add the cast to the
		 * iterator }
		 */
...
class SetFunctionality extends MainArray {
	public void addtoset(ArrayFunctionality s1, ArrayFunctionality s2, ArrayFunctionality s3, ArrayFunctionality s4,
			List<String> re) {
		...
		s.forEach(a -> {
			System.out.println(
					"name-> " + a.name + " gender-> " + a.gender + " age-> " + a.age + " salary-> " + a.salary);
		});
		Set<String> set = new HashSet<String>(re); // created set and adding element from list of string
													// //typeconversion from other collection
...
class Mapping {
	...
	public void comparision(Map<Integer, String> m) {
		...
		// then using for each method to perform the printing action on the element
	...
	public void hashtable(Map<Integer, String> m) {
		...
		System.out.println("\n\nusing compute if absent: " + m1.entrySet());
		System.out.println("\n\nsorting hashtable after computing in reverse  according to keys: ");
	...
	public void treemap(Map<Integer, String> m) {
		...
		// treemap is a class under sortedmap interface which is the subinterface of map
		// interface
		...
		// same line can be written by creating the object of navigable interface by
		// which we need not to add the cast while declaring
		NavigableMap<Integer, String> n = new TreeMap<Integer, String>(m1);
		// here navigable map is the subinterface of map interface in which we are using
		// the tree map class so that we can use some functionality
	...
	public void linkedhashfunctionality(ArrayFunctionality s1, ArrayFunctionality s2, ArrayFunctionality s3,
			ArrayFunctionality s4) {
		...
		m.forEach((k, v) -> {
			System.out.println("Keys: " + k + "   names: " + v.name + "   geneder: " + v.gender + "   age: " + v.age
					+ "   salary: " + v.salary);
		});
...
class Enumoperation {
	...
	public void operation(ArrayFunctionality s1, ArrayFunctionality s2, ArrayFunctionality s3, ArrayFunctionality s4) {
		...
		e.forEach((k, v) -> {
			System.out.println("Keys: " + k + " names: " + v.name + " gender: " + v.gender + " age: " + v.age
					+ " salary: " + v.salary);
		});
...
class CollectionOperation {
	public void operation(List<String> s) {
		Collections.addAll(s, "collect 1", "collect 2", "collect 3"); // it means to add the string inside list of
																		// string
		System.out.println("\n\n");
		s.forEach(a -> {
			System.out.println("collection data: " + a);
		});
		...
		Collections.sort(s, Collections.reverseOrder()); // first it will contain the list into the reverse order and
															// then it sort the element
...
class MainArray {
	...
	public static void serialization(List<ArrayFunctionality> l) {
		try {
			...
			ArrayFunctionality a = (ArrayFunctionality) o1.readObject(); // as the data is of type arrayfunctionality
																			// type therefore creating the variable of
																			// same type
			System.out.println(a);
			o1.close();
			i.close();
	...
	public static List<ArrayFunctionality> addtolist(ArrayFunctionality arr, ArrayFunctionality arr1,
			ArrayFunctionality ar2, ArrayFunctionality arr3) {
